...
<#if baseColumnList>
    ...
    <!-- 根据查询条件获取id列表 -->
    <select id="listIdsByQuery" resultType="java.lang.Long" parameterType="${package.Query}.${table.classBodyName}Query" >
        SELECT <#list table.fields as field><#if field.keyFlag>${field.name}</#if></#list>
        FROM ${table.name}
        <where>
            is_deleted = 0
            <include refid="query_sql" />
    ...
    <sql id="order_by_column_sql">
        <choose>
            ...
            <when test="query.sortColumn == 'createDate'">
                create_date
            </when>
            <when test="query.sortColumn == 'updateDate'">
                update_date
            </when>
            <otherwise>
                id
